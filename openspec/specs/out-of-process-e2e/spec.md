...
Define an out-of-process end-to-end verification tier that exercises the real published Docker image as a running process, driven over HTTP, with only the outermost third-party systems stubbed — proving the full acquisition flow works across the process boundary before the image is published.
