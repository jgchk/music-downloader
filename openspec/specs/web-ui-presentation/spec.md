...
Define the web UI's presentation as a semantic landmark skeleton that CSS alone restyles — theme **and** layout — via a two-tier design-token layer and interchangeable `data-skin` skins, together with the accessibility guarantees that keep the skeleton sound under any skin and a user-facing, persisted skin switch. Provenance: `web-swappable-skin-theming`.
