...
Gate the product's web interface behind Plex-account sign-in with share-is-approval authorization: the Plex PIN login flow (full-page redirect, browser-bound PINs), stateless signed sessions with fixed expiry, the route gate and its exact open set (`/login`, `/login/callback`, `/health`), and the fail-closed, no-credential-retention properties that keep the gate safe under misconfiguration and plex.tv outages. Provenance: `add-plex-auth-gate`.

## Requirements

### Requirement: The web interface requires an authorized session

The web interface SHALL require a valid session on every server route except the login flow (`/login`, its callback) and the health endpoint. An unauthenticated GET or HEAD request SHALL be redirected to the login page; any other unauthenticated request (form actions and other writes) SHALL be refused without side effects. A valid session SHALL grant the base interface — submission, cancellation, selection, and review resolution — regardless of role; per-user distinctions SHALL exist only as owner-gated privileged actions decided through the authorization seam (see `web-authorization`). No route or action is gated by role today; the seam exists so that one can be.
...
Authorization SHALL be decided by asking plex.tv, with the logging-in user's own token, whether the account can see a **server** whose machine identifier equals the configured `PLEX_SERVER_MACHINE_ID`: the matching resource entry SHALL both carry the configured machine identifier and declare that it provides a server. A resource entry that matches the identifier but does not provide a server — a player or other device, whose identifiers are client-chosen — SHALL NOT satisfy the check. This requirement NARROWS admission; it does not authenticate the server, because a resource's declared capabilities are self-asserted exactly as its identifier is — closing that residual requires pinning the owner by account identity, which this capability does not yet require. There SHALL be no user database, allowlist, or per-user permissions. The matched entry's ownership flag SHALL be read at this moment as the session's role source (see `web-authorization`). A plex.tv failure or unreachability SHALL surface as a modeled infrastructure error on the login page and SHALL never result in a grant.

#### Scenario: Shared account is admitted

- **WHEN** the authenticated Plex account's accessible resources include a server providing the configured machine identifier
...
- **WHEN** the authenticated account's resources contain an entry carrying the configured machine identifier that does not declare it provides a server
...
A session SHALL be a signed, HttpOnly cookie carrying the Plex account identity, the session's role, and a fixed expiry 7 days from login, verified per-request by pure computation against the configured `SESSION_SECRET` — no server-side session state and no per-request plex.tv call. Activity SHALL NOT extend a session; re-login re-runs the membership check, making cookie lifetime the re-verification cadence. A validly signed cookie without a role field SHALL decode as a guest session (see `web-authorization`); role SHALL change only by logging in again.
...
- **THEN** the role is available to the authorization seam from the cookie alone, with no store read or upstream call
