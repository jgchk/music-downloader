...
- [ ] 2.1 Create the copy module (single source of all timeline/status strings): entry-kind →
...
- [ ] 2.2 Rewrite `AcquisitionDetail.svelte`'s timeline rendering to consume the copy module:
...
- [ ] 2.3 Layer-2 disclosure: per-entry native `<details>/<summary>` rendered only when the entry
...
- [ ] 3.1 Timestamp rendering: `<time datetime title>` on every entry, hybrid display (relative
...
- [ ] 3.2 Synthesized in-progress row: derive from downloader status (+ importer status after
  ...
  empty).
- [ ] 3.3 Fold the ProgressBar into the downloading in-progress row, including the
  progress-unavailable sentence inside the row; remove the standalone placement; tests.
- [ ] 3.4 Retire "Nothing has happened yet."; defensive no-entries fallback becomes the D5
  neutral string; import-unavailable indication re-worded to the unified voice; tests.

## 4. Web: page register and titles (D9, D10)

- [ ] 4.1 Status line rework: badge + human phrase map (D9), pluralized counts with zero-count
  segments omitted; no raw enums anywhere on the page; tests across statuses.
- [ ] 4.2 Replace the orphan location/enum paragraph with the labeled library-location line
  (present only when `location` exists); failure accounts live only in the timeline's terminal
  row; tests.
- [ ] 4.3 Queue sidebar: pluralize attempts, hide at zero; title fallback chain (resolved target →
...
- [ ] 5.1 Freshness-driver seam: a page-layer module with `start/stop` semantics wrapping
...
- [ ] 5.2 Amend the "freshness is page-navigation freshness" design comment in
  `+layout.server.ts` to record the revision and the SSE-successor intent.

## 6. Skins (D11)

- [ ] 6.1 Skeleton/token layer: timeline anatomy hooks (marker slot, content, time, pending
...
- [ ] 6.2 Theme the anatomy in all three skins — forum first (receipt-like finishing pass), then
  glass and terminal; no browser-default-styled timeline under any skin; visual pass over real
  data on all three.
- [ ] 6.3 Accessibility checks: entry text carries state without color, `<details>` keyboard
  operation, `time` metadata present — under each skin.

## 7. Verification and ship

- [ ] 7.1 Update affected e2e/Playwright expectations (detail page text assertions, timeline
  presence) and the out-of-process e2e where it asserts history copy.
- [ ] 7.2 `pnpm check` clean at 100% coverage; contract/drift tiers green.
