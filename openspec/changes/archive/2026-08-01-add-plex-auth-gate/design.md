...
**D4 — Stateless signed cookie, fixed 7-day expiry, logout included.** HttpOnly/Secure/SameSite=Lax cookie carrying signed claims (Plex account id, username, expiry), named with the `__Host-` prefix (as is the login-pin binding cookie): the browser then refuses any Domain-scoped plant, so a sibling `*.jake.cafe` service — compromised or stale — cannot shadow the session cookie (a persistent sign-out DoS) or restore the login-fixation vector by planting a binding for its own PIN. The prefix demands Secure + Path=/ on every set AND delete (a Secure-less `__Host-` deletion is rejected and the cookie survives — the logout journey spec guards this). Fixed—not sliding—expiry is what makes cookie lifetime the re-verification cadence: every session is at most 7 days from a fresh share check, so unsharing someone actually locks them out. Sliding expiry was rejected because an active unshared user would never be re-checked; sliding-with-revalidation deferred as machinery. `SESSION_SECRET` rotation is the everyone-out-now lever. Plex being down never blocks existing sessions (nothing to check server-side); it only blocks new logins — fail closed.
...
**D9 — Gate surface: an exact open set — `/login`, `/login/callback`, `/health`.** The `handle` gate redirects unauthenticated GET/HEAD requests to `/login` and refuses any other unauthenticated request (form actions) with a 403; a future route born under `/login/` starts out gated until deliberately listed. `/health` stays open (Komodo, `/ship` verification, uptime checks — no session, no side effects). Framework-served static assets are not secrets; the gate covers server routes, which is where every fact lives.
