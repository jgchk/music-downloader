...
Mint (W3C trace-id), adopt-vs-mint, causation chaining, the causation parser, and the branded `CorrelationId` all move to `packages/eventing`, bound per context via `createCorrelation(contextName)`. The wire **envelope schemas stay per context** in each `interfaces/contracts/`: they are the producer's published contract read by a tolerant reader, which is model, not mechanism — the shared constant is the id FORMAT alone. The tolerant-reader property earns its keep on the event payload contract, not on the never-fatal observability sidecar; keeping ceremonial per-context envelope schemas would preserve exactly the duplication this change ends. This amends D13's conclusion while keeping its core claim: the envelope still crosses the seam as data, the facades still take a plain `StoryId` string. *Alternative — share the mechanics but keep a per-context brand:* rejected as letter-over-spirit. TypeScript brands are structural, so two identically-tagged declarations were already mutually assignable — per-context branding implied an isolation it never bought.
...
- [The two per-context adapters are byte-identical again, which is the very shape this change cites as evidence of accidental duplication] → accepted and distinguished: unlike the mechanism they replaced, these are ALLOWED to diverge (each side's own wire shape and its own reading of the other's error vocabulary), and their one cross-seam token — the permanent feed-error kind — is pinned from both roles by the contract tier against each module's `seam-contract.json`. A future extraction must not point this change's argument at them.
- [`globalSeq` became `position` on drain log lines, so an operator filter on the old field goes quiet] → accepted: the generic module cannot carry a consumer's field name. It is the one observable surface this refactor changes, recorded here and in the proposal rather than discovered in a dashboard.
