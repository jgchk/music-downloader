...
- [ ] 1.2 Implement: `GroupedRelease` retains the group's identity title (release-group title; release title for the singleton fallback); apply the `titled`-set decision from design D1 ahead of the margin guard; reuse `normalizeTitle`.

## 2. Gate + fidelity

- [ ] 2.1 `pnpm check` green (100% coverage on the new branches); doc comments on `releaseCandidateIds` updated to describe the preference and its fail-safe ties.
