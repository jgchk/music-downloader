...
- **THEN** the BFF dispatches the downloader facade's submit command in-process with default policies and the UI confirms the new acquisition with its identifier
...
- **THEN** the submit command carries that edition choice and the UI confirms the new acquisition
...
- **THEN** the UI surfaces the failure's message and no acquisition is created

#### Scenario: No-JS fallback still submits

- **WHEN** a user without JavaScript opens the request page
- **THEN** a native form submission can still create an acquisition through the same facade contract
