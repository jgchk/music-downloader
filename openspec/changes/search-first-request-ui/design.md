...
4. **One search request from the browser; fan-out on the server.** The BFF/facade read performs the three MB entity searches (and caching) server-side. Debounce (600 ms) + Enter-immediate + in-flight abort in the client; short-TTL, size-bounded in-memory cache keyed by URL on the server, with concurrent identical reads sharing one request. Tracklists are a separate lazy read (avoid N+1). **No request-rate ceiling** — an earlier draft of this decision called for one, and implementing it showed why not: a minimum-interval queue would serialize a search's three entity reads into multi-second waits on a surface a person is typing into. The stewardship requirement was rewritten to match what actually protects the upstream (identification, caching, sharing, bounded fan-out). *Alternative rejected:* browser→MB direct (CORS works but violates stewardship, leaks vocabulary, and can't cache across household users).
5. **Cover art: server proxy endpoint with two-state caching.** `/cover-art/<entity>/<mbid>?size=` reads CAA's JSON manifest (the only part with a shape a contract test can hold it to), then fetches the front image it names — over https, from an allow-listed host, re-served with the declared image type and `nosniff`. Art and confirmed absence are cached (with different lifetimes); an unreachable archive never is. The UI renders an initials placeholder in the artwork slot, which the image paints over once it loads — no `onerror` is needed, because an image that never loads paints nothing. *Alternative rejected:* hotlinking CAA from the browser (slow, 404-noisy, third-party requests from every client) or eagerly resolving art URLs inside search results (couples reads, serializes latency).
6. **The page is client-rendered search over a native-form skeleton.** The SSR baseline renders the search input plus a disclosure holding a native artist/title form that posts to the existing action, satisfying the no-JS scenario; JS progressively upgrades to search-as-you-type, filters, and the detail surface. Every result's Request is itself a real form post, so requesting needs no script once results are on screen. Components read semantic tokens only; the drawer becomes a bottom sheet under ~640 px via CSS alone so skins keep working unchanged. *Alternative rejected:* keeping the old mode-select form as a parallel page (two request surfaces to maintain, and the audit in `e2e-blackbox-blast-radius` says scraping tiers break silently when copy/flows fork).
