...
**D3 — Full-page redirect PIN flow, PIN created on form POST.** Login page renders statically; submitting the form creates the PIN server-side and redirects the tab to `app.plex.tv/auth#?...&forwardUrl=<origin>/login/callback`; the callback checks the PIN once and sets the cookie. Rejected: Overseerr's popup-and-poll (client-side popup lifecycle + poll loop + popup-blocker fallback — strictly more code and test surface for the same Plex approval screen), and PIN-on-page-render (would let drive-by GETs make us call plex.tv; the form POST also gives nginx a natural `limit_req` target).
