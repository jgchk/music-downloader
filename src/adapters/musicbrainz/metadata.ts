...
import type { ScoredEntry } from './mapping.js';
...
export class MusicBrainzMetadata implements MetadataPort {
  ...
  private async resolveReleaseByDescriptor(
    ...
    const json = (await this.getJson(this.searchUrl('release', query))) as
      { releases?: readonly ScoredEntry[] } | undefined;
  ...
  private async resolveRecordingByDescriptor(
    ...
    const json = (await this.getJson(this.searchUrl('recording', query))) as
      { recordings?: readonly ScoredEntry[] } | undefined;
  ...
  /** GET and parse JSON; `undefined` for 404 (not found → unresolved), throw for other non-2xx. */
