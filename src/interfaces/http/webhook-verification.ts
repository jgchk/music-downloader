...
/**
 * Standard Webhooks-style delivery verification for the inbound verdict receiver (fulfillment-
 * external-verdict D4) — the mirror of the outbound dispatcher's signing: `v1,` + base64
 * HMAC-SHA256 over `id.timestamp.body` with the shared `whsec_<base64>` secret, plus a replay
 * window on `webhook-timestamp`. Unsigned or stale deliveries are rejected as values before any
