...
async function seed(history: readonly DownloadEvent[], streamId = 'acq-1'): Promise<void> {
...
function seedLegacy(history: readonly DownloadEvent[], streamId = 'acq-1'): void {
  const current = store.all().filter((entry) => entry.streamId === streamId).length;
  store.seedLegacy(streamId, current, history, { acquisitionId: streamId, occurredAt: 't' });
}

function storedOfType(type: DownloadEvent['type']): StoredEvent {
...
const importedThenFulfilled = (cands: readonly ReturnType<typeof matchingCandidate>[]) => [
  ...
  { type: 'DownloadFulfilled' as const, location: '/lib/kid-a' },
];
...
describe('Reactor.start', () => {
  ...
  it('re-drives an already-checkpointed in-flight download after a restart', async () => {
    ...
    expect(streamEventTypes('acq-1')).toContain('TryStarted');
  });

  it('re-drives a watch whose stream already recorded TryStarted (mid-transfer restart)', async () => {
    // The stream's LAST event mid-download is TryStarted; its reaction must re-derive the
    // ensure-start or a restarted process would never rebuild the supervisor's watch
    // (nonblocking-download-observation D3).
    await seed(startedHistory([matchingCandidate('a')])); // ends at TryStarted (globalSeq 6)
    await checkpoints.save(REACTOR_CONSUMER, 6);
    const ports = stubPorts();
    await reactor(ports).start();
    expect(ports.download.start).toHaveBeenCalledOnce();
    // The absorbed duplicate report appends nothing: TryStarted appears exactly once.
    expect(streamEventTypes('acq-1').filter((type) => type === 'TryStarted')).toHaveLength(1);
  });
...
describe('Reactor.process', () => {
  ...
  it('advances the checkpoint when an effect follow-on is rejected as a stale domain transition', async () => {
    ...
    await seed(resolvedHistory()); // DownloadRequested (seq 1) then TargetResolved (seq 2)
    const requested = storedOfType('DownloadRequested');
...
describe('Reactor — per-stream parking (reactor-durability D1)', () => {
  ...
  it('processes another download immediately while one is parked (the isolation scenario)', async () => {
    await seed(requestedHistory(), 'acq-1');
    await seed(requestedHistory(), 'acq-2');
    const resolve = vi
      .fn()
  ...
  it('keeps draining other acquisitions and due retries while a download is in flight (the incident shape)', async () => {
    ...
    // freezing every other download's searches, resolutions, and parked retries. The start
    ...
    const resolve = vi
      .fn()
      // acq-1's replayed DownloadRequested resolves; its follow-on is stale and advances past.
    ...
    expect(streamEventTypes('acq-1')).toContain('TryStarted'); // in flight, unsettled
...
describe('Reactor — ordering under park (no-leapfrog invariant)', () => {
  it('queues later events of a parked stream and dispatches them in order after the park resolves', async () => {
    ...
    await seed([{ type: 'DownloadCancelled' }]);
    await r.drain();
  ...
  it('leaves an event the drain has not reached to the drain, dispatching it exactly once', async () => {
    ...
    await r.start(); // parks acq-1 at the CandidateSelected download
    await seed([{ type: 'TryFailed', candidate: a.identity, reason: 'Stalled' }]); // seq 6
...
describe('Reactor — budget exhaustion lands somewhere modeled (reactor-durability D2)', () => {
  ...
  it('dead-letters an effect with no modeled failure and records the owning stream', async () => {
    ...
    expect(lines.join('')).toContain('dead-lettered and download stalled');
    r.stop();
  });
  ...
  it('dead-letters an exhausted Validate — a verdict cannot be fabricated', async () => {
    const a = matchingCandidate('a');
    await seed(validatingHistory([a])); // ends TryCompleted -> Validate effect
  ...
  it('degrades an exhausted AbortDownload to the modeled cancelled rejection', async () => {
    const a = matchingCandidate('a');
    await seed([...selectedHistory([a]), { type: 'DownloadCancelled' }]);
  ...
  it('treats a degrade rejected by the domain as landed (the stream already settled it)', async () => {
    ...
    // degrade's RecordMetadataFailed is an IllegalTransition — the download already terminal.
    await seed(requestedHistory());
    const resolve = vi.fn(() => errAsync(infraError('mb', 'down')));
    const r = reactor(stubPorts({ metadata: { resolve } }), { retryPolicy: TIGHT_BUDGET });
    await r.start();
  ...
  it('marks the download stalled on dead-letter and clears it when the stream resumes', async () => {
    const a = matchingCandidate('a');
    await seed([
      ...selectedHistory([a]),
      { type: 'TryFailed', candidate: a.identity, reason: 'Stalled' },
  ...
  it('stays stalled when clearing the resolved dead letters fails, and retries later', async () => {
    ...
    await seed([{ type: 'DownloadCancelled' }]); // absorbed no-op event, drives successfully
    await r.drain();
    expect(stalled.isStalled('acq-1')).toBe(false);
    r.stop();
  });
...
describe('Reactor — startup re-drive (reactor-durability D3)', () => {
  ...
  it('re-derives and dispatches the pending effect of every non-terminal stream', async () => {
    ...
    expect(
      streamEventTypes('acq-downloading').filter((type) => type === 'TryStarted'),
  ...
  it('passes over a terminal stream even when its last event still derives an effect', async () => {
    // Cancelled is terminal, yet reacting to its DownloadCancelled derives an AbortDownload for
    // the transfer that was in flight. Terminal means settled: re-driving it would re-issue an
    // abort for every long-cancelled download on every boot. Reclaiming a source resource
    // orphaned by a crash is the resource sweep's job, not the re-drive's.
    await seed([...selectedHistory([matchingCandidate('a')]), { type: 'DownloadCancelled' }]);
...
describe('Reactor.process — reacts against the state as of the event (prefix fold)', () => {
  ...
  it('reacts to a co-emitted non-final event against its own post-state, not the batch successor', async () => {
    // Imported is co-emitted with DownloadFulfilled; reacting to Imported folds only through
  ...
  it('absorbs an outcome that lands before the start report (the re-attach-to-settled race)', async () => {
    ...
    expect(types.filter((type) => type === 'TryCompleted')).toHaveLength(1);
    expect(types).not.toContain('TryStarted'); // the late report was absorbed, not recorded
