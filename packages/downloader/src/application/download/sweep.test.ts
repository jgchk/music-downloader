...
async function seed(acquisitionId: string, isTerminal: boolean): Promise<void> {
  ...
  const history = isTerminal
    ? [...selectedHistory([a]), { type: 'DownloadCancelled' as const }]
    : selectedHistory([a]);
...
describe('SourceResourceSweep', () => {
  it("removes a terminal download's resource and marks it removed", async () => {
  ...
  it("leaves an in-flight download's resource untouched", async () => {
