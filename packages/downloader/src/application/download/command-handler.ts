...
import type { DomainError } from '../../domain/download/download.js';
...
function attemptCommand(
  ...
  return dependencies.store.readStream(acquisitionId).andThen((stored) => {
    const download = Download.fromHistory(stored.map((entry) => entry.event));
    const decision = download.execute(command);
