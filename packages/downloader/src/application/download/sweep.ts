 ...
 * source. For every live ledger row whose owning download has reached a terminal state, it removes
 * the resource from the source and marks the row removed. Rows owned by an in-flight download are
...
export class SourceResourceSweep {
  ...
  private async sweepOne(resource: SourceResource): Promise<void> {
    ...
    if (!terminal.value) return; // an in-flight download still owns this — leave it to the reactor
  ...
  private isTerminal(acquisitionId: string): ResultAsync<boolean, InfraError> {
    return this.dependencies.store
      ...
      .map((stored) => Download.fromHistory(stored.map((entry) => entry.event)).isTerminal);
  }
}
