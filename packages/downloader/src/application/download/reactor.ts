 ...
 * poisoned download never stalls the rest (reactor-durability D1); the retry scheduler runs on
...
export class Reactor {
   ...
   * live drain can never race on the same download.
   */
   ...
   * never race a live drain over the same download (a check-then-act re-attach hazard).
   */
  ...
  private async redriveStream(streamId: string, events: readonly StoredEvent[]): Promise<void> {
    ...
    const download = Download.fromHistory(events.map((entry) => entry.event));
    if (download.isTerminal) return;
    const last = events.at(-1)!;
    if (download.reactTo(last.event).length === 0) return; // nothing pending (e.g. paused)
    ...
    scope.logger.info(
      { acquisitionId: streamId, phase: download.phase },
  ...
  async process(stored: StoredEvent): Promise<void> {
    ...
    if (outcome.kind === 'retry') {
      ...
      // A stalled download's stream was driven successfully again (a cancellation, an operator
  ...
  private async dispatchEvent(
    ...
    const download = Download.fromHistory(prefix.map((entry) => entry.event));
    for (const effect of download.reactTo(stored.event)) {
