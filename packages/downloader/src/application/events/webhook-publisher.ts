...
/**
 * The durable webhook publisher (change: acquisition-outbound-events). The event store IS the
 * transactional outbox: domain events are durably appended before anything else happens, so
 * publishing is just one more checkpointed consumer of the global stream — the reactor's shape,
 * with a per-subscriber consumer name. Per subscriber, events are delivered in global order,
 * at-least-once: the checkpoint advances only on acknowledged delivery, an undelivered event is
 * never skipped ahead of (order over throughput), and a crash or dead subscriber redelivers from
 * the checkpoint on the next cycle/restart (convergence over loss). Subscribers are isolated —
 * one slow or dead URL holds only its own checkpoint.
 *
...
class SubscriberWorker {
  ...
  private async publish(stored: StoredEvent): Promise<boolean> {
    ...
    if (stream.isErr()) {
      this.deps.logger.error(
        { url: this.url, acquisitionId: stored.streamId, err: stream.error },
    ...
    if (rendered.isErr()) {
      ...
      this.deps.logger.error(
        { url: this.url, acquisitionId: stored.streamId, err: rendered.error },
