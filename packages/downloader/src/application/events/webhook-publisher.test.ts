...
function fulfilledHistory(): AcquisitionEvent[] {
...
async function seed(history: readonly AcquisitionEvent[], streamId = 'acq-1'): Promise<void> {
  await store.append(streamId, 0, history, { acquisitionId: streamId, occurredAt: 't' });
}
...
describe('WebhookPublisher.start', () => {
  ...
  it('renders from the stream prefix as of the event, not the whole stream', async () => {
    await seed(fulfilledHistory());
    await seed(requestedHistory(), 'acq-2'); // later, unrelated stream noise
    await publisher().start();

    const fulfilled = store.all().find((entry) => entry.type === 'AcquisitionFulfilled')!;
    const [stored, prefix] = vi.mocked(mapping.render).mock.calls[0]!;
    expect(stored.globalSeq).toBe(fulfilled.globalSeq);
    expect(prefix.at(-1)!.globalSeq).toBe(fulfilled.globalSeq);
    expect(prefix.every((entry) => entry.streamId === 'acq-1')).toBe(true);
    expect(prefix.every((entry) => entry.version <= fulfilled.version)).toBe(true);
  });
...
describe('WebhookPublisher delivery semantics', () => {
  ...
  it('preserves order: an undelivered event blocks later events for that subscriber', async () => {
    await seed(fulfilledHistory(), 'acq-1');
    const firstFulfilledSeq = lastSeq();
    await seed(fulfilledHistory(), 'acq-2');
    ...
    expect(deliveries).toHaveLength(0);
    expect(checkpoints.peek(consumerA)).toBe(firstFulfilledSeq - 1);
  });
  ...
  it('does not attempt delivery when rendering fails, and holds the checkpoint', async () => {
    ...
    const brokenMapping: PublishedEventMapping = {
      publishes: () => true,
      render: () => err({ kind: 'RenderError', eventType: 'acquisition.fulfilled', message: 'x' }),
    };
