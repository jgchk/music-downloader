...
const stateByPhase: Record<DownloadPhase, DownloadState> = {
  Empty: foldEvents([]),
  ...
  Exhausted: foldEvents([...selectingHistory, { type: 'DownloadExhausted' }]),
  Cancelled: foldEvents([...selectedHistory([a]), { type: 'DownloadCancelled' }]),
};
...
const allEvents: DownloadEvent[] = [
  ...
  { type: 'TryStarted', candidate: a.identity },
  { type: 'TryCompleted', candidate: a.identity, files: [] },
  { type: 'TryFailed', candidate: a.identity, reason: 'Stalled' },
  { type: 'CandidateRejected', candidate: a.identity },
  {
  ...
  { type: 'DownloadFulfilled', location: '/x' },
...
const NON_TERMINAL: readonly DownloadPhase[] = [
  'Empty',
  'Pending',
...
const legalSources: Record<DownloadEvent['type'], readonly DownloadPhase[]> = {
  DownloadRequested: ['Empty'],
  ...
  TryStarted: ['Downloading'],
  TryCompleted: ['Downloading'],
  TryFailed: [],
  CandidateRejected: ['Downloading', 'Validating', 'Cancelled'],
  ValidationPassed: ['Validating'],
  ValidationFailed: [],
  Imported: [],
  DownloadFulfilled: ['Importing'],
  FulfillmentRejected: ['Fulfilled'],
  DownloadExhausted: ['Selecting'],
  ImportConflicted: ['Importing'],
  DownloadCancelled: NON_TERMINAL,
};

const ALL_PHASES = Object.keys(stateByPhase) as DownloadPhase[];
...
describe('evolve — fulfilment retains the ladder-resume context (defeasible Fulfilled, D3)', () => {
  type Fulfilled = Extract<DownloadState, { phase: 'Fulfilled' }>;
  ...
  it('folds the whole revival batch through the existing rejection/selection cases', () => {
    ...
    expect((state as Extract<DownloadState, { phase: 'Downloading' }>).rejected).toHaveLength(1);
  });
...
describe('evolve — cancellation edge cases', () => {
  it('cancels an empty download into a terminal cancelled state with zeroed progress', () => {
    const cancelled = foldEvents([{ type: 'DownloadCancelled' }]) as Extract<
      DownloadState,
      { phase: 'Cancelled' }
    >;
...
describe('evolve — cancellation retains and settles the mid-download candidate', () => {
  const cancelledInFlight = foldEvents([
    ...selectedHistory([a]),
    { type: 'DownloadCancelled' },
  ]) as Extract<DownloadState, { phase: 'Cancelled' }>;
...
describe('evolve — manual edition selection pauses and resumes', () => {
  ...
  it('cancelling while awaiting selection folds through the existing cancel path', () => {
    const state = foldEvents([...awaitingSelectionHistory(), { type: 'DownloadCancelled' }]);
