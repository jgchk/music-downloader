...
import type { DownloadEvent, EditionCandidate } from './events.js';
...
/**
 * The download aggregate (D1): the single public face of the download domain. It wraps the
 * functional decider — `decide`/`evolve`/`react` and the folded `DownloadState` stay private
 * module internals of this folder, reachable only through this class. The aggregate is pure and
 * immutable: rehydrate a history with {@link Download.fromHistory}, then observe it — nothing
 * here performs I/O or mutates.
 *
 * Commands, events, {@link DomainError}, {@link Effect}, and {@link DownloadPhase} remain the
...
export type { DownloadPhase } from './state.js';
 ...
 * write-model `DownloadState`, which the aggregate never exposes.
 */
export interface DownloadSnapshot {
  readonly phase: DownloadPhase;
  /**
   * The transfer is live at the source: the current attempt's enqueue was accepted
   * (`TryStarted` folded). The download's own determination — consumers read it instead
 ...
 * settled) — so a cancelled-in-flight or exhausted download reports none.
 */
function currentIdentityOf(state: DownloadState): CandidateIdentity | undefined {
...
export class Download {
  private constructor(private readonly state: DownloadState) {}

  /** Rehydrate an aggregate by folding its event history (the replay path). */
  static fromHistory(events: readonly DownloadEvent[]): Download {
    return new Download(foldEvents(events));
  }

  /** Run a command against the current state: the events to append, or a `DomainError`. */
  execute(command: DownloadCommand): Result<readonly DownloadEvent[], DomainError> {
    return decide(command, this.state);
  }

  /** The reflex: zero or more effect descriptions for an event applied to this state. */
  reactTo(event: DownloadEvent): readonly Effect[] {
  ...
  get snapshot(): DownloadSnapshot {
    ...
    return {
      ...
      // destructures it away; `DownloadCancelled` rebuilds field by field), and a `{ ...state }`
