...
export const sampleRequest: DownloadRequest = {
  kind: 'musicbrainz',
  mbid: asMbid('mbid-1'),
  targetType: 'album',
};
...
export function defaultPolicies(overrides: Partial<DownloadPolicies> = {}): DownloadPolicies {
  return {
...
export const sampleGroupRequest: DownloadRequest = {
  kind: 'release-group',
  mbid: asMbid('rg-1'),
  targetType: 'album',
};
...
export function startedHistory(
  ...
  return [
    ...
    { type: 'TryStarted', candidate: selected.identity },
  ];
}
...
export function validatingHistory(
  ...
  return [
    ...selectedHistory(candidates, policies),
    { type: 'TryCompleted', candidate: selected.identity, files: sampleFiles },
  ];
}
 ...
 * `DownloadFulfilled` names the fulfilled candidate (fulfillment-external-verdict D3).
 */
export function fulfilledHistory(
  ...
  return [
    ...
    { type: 'DownloadFulfilled', location, candidate: selected.identity },
  ];
}
