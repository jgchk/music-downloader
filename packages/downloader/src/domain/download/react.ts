...
import type { TryPolicy, MatchPolicy } from '../policy/policies.js';
import type { Target } from '../target/target.js';
import type { DownloadEvent, DownloadRequest, DownloadedFile } from './events.js';
 ...
 */
export type Effect =
  | { readonly type: 'ResolveMetadata'; readonly request: DownloadRequest }
  | { readonly type: 'Search'; readonly target: Target; readonly round: number }
  | { readonly type: 'Download'; readonly candidate: Candidate; readonly policy: TryPolicy }
  | {
 ...
 *
 * The phase narrowings below are TypeScript refinements over the `DownloadState` union; for a
 ...
 */
export function react(event: DownloadEvent, state: DownloadState): readonly Effect[] {
  switch (event.type) {
    ...
    case 'EditionSelected': {
      ...
      // (whose type pins album), so an awaiting download can never hold anything else — and a
    ...
    // the decider can mint. Deleting or emptying this arm falls through to `TryStarted`,
    ...
    // download, where the tolerant fold ignores the event; that divergence is documented in
    // `state.property.test.ts` and deliberately not asserted anywhere.) The arms stay separate
    // because this one reads the full candidate its event carries (D3), while `TryStarted`
    ...
    case 'TryStarted': {
      // Level-triggered ensure: the same Download effect the selection fired. Re-delivering it
      // for work already in flight is an idempotent no-op; after a restart this re-derivation is
      // what resumes the download — the reactor's re-drive keys off the LAST event, and
      // mid-download that is this one (nonblocking-download-observation D3).
      return state.phase === 'Downloading'
