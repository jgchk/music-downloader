...
import type { DownloadCommandType } from './commands.js';
import type { DownloadEvent, DownloadEventType } from './events.js';
import { initialState, isTerminal } from './state.js';
import type { DownloadPhase } from './state.js';
import {
  arbBlindCommandStep,
  arbCommandSequence,
...
/**
 * Property suite for the download decider (decider-properties D1): universally-quantified
...
const ALL_PHASES: readonly DownloadPhase[] = [
  'Empty',
  'Pending',
...
describe('the generators cover the decider’s whole command and event surface', () => {
  it('generates every event variant, and only under its own key', () => {
    for (const [type, arbitrary] of Object.entries<fc.Arbitrary<DownloadEvent>>(
      eventArbitraryByType,
    )) {
      const samples = fc.sample(arbitrary, { numRuns: 10, seed: propertyRun.seed });

      expect(samples.map((event) => event.type)).toEqual(
        Array.from({ length: samples.length }, () => type as DownloadEventType),
      );
    }
  });
  ...
  it('generates every command variant, and only under its own key', () => {
    for (const [type, arbitrary] of Object.entries<fc.Arbitrary<CommandStep>>(
      ...
      expect(samples.map((step) => step(initialState).type)).toEqual(
        Array.from({ length: samples.length }, () => type as DownloadCommandType),
      );
    }
  });
});
...
describe('a settled download converges: late effect results are absorbed, not refused', () => {
    ...
    assertProperty(
      fc.property(arbCommandSequence, arbBlindCommandStep, (plan, intruder) => {
        for (const state of driveCommands(plan).states) {
          ...
          // Events are lawful here — a cancelled download whose in-flight transfer finally
    ...
    assertProperty(
      fc.property(arbCommandSequence, (plan) => {
        for (const state of driveCommands(plan).states) {
          ...
          expect(['CandidateSelected', 'SearchRequested', 'DownloadExhausted']).toContain(third);
          expect(rest).toEqual([]);
          ...
          // A verdict naming someone else is stale news about a candidate this download never
...
describe('the sweep is not vacuous: the generated plans reach the whole lifecycle', () => {
  it('visits every phase and exercises every command across the pinned corpus', () => {
    const phasesSeen = new Set<DownloadPhase>();
    const commandsSeen = new Set<DownloadCommandType>();
