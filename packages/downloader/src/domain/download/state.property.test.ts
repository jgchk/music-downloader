 ...
 */
const KNOWN_PHASE_TABLE: Record<DownloadPhase, true> = {
  Empty: true,
  Pending: true,
...
const arbHistory: fc.Arbitrary<readonly DownloadEvent[]> = arbAnyHistory;
...
describe('evolve is total over every event a stream can contain', () => {
  ...
  it('never lets any event — lawful or not — reopen an absorbed download', () => {
    ...
    assertProperty(
      fc.property(arbHistory, (history) => {
        ...
        // or conflicted download back into an active phase.
        let settled: DownloadPhase | undefined;
...
describe('the edition menu belongs to one phase and no other', () => {
  it('carries `candidates` on no state but AwaitingManualSelection', () => {
    ...
    assertProperty(
      fc.property(arbHistory, (history) => {
        // `Download.snapshot` reads the menu with `'candidates' in state`, and that is only the
        // same question as the phase because every exit from AwaitingManualSelection drops the key:
        // `EditionSelected` destructures it away, and `DownloadCancelled` rebuilds the state
        ...
        // the UI would render as "choose an edition" for a dead download. The compiler does not
