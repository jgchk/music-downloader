 ...
 * on {@link DownloadPhase} so that each phase carries exactly the fields valid in it — invalid
...
export interface DownloadingState extends Targeted {
  ...
  /** True once the source accepted the enqueue (`TryStarted`) — the transfer is live. */
 ...
 * the `DownloadFulfilled` event names its candidate; a legacy fulfilment folds without it and
...
/**
 * What a cancellation retained of the download's transfer — at most one candidate, and the two
...
const TERMINAL_PHASES: ReadonlySet<DownloadPhase> = new Set<DownloadPhase>([
  'Fulfilled',
  'Exhausted',
  'Cancelled',
  'MetadataFailed',
...
function progressOf(state: DownloadState): Progress {
...
export function evolve(state: DownloadState, event: DownloadEvent): DownloadState {
  switch (event.type) {
    ...
    case 'CandidateRejected': {
      // A cancelled download whose mid-download candidate has now settled: drop `pending` so the
    ...
    case 'FulfillmentRejected': {
      // The revival edge (D1/D2): a Fulfilled download with retained context re-enters the
...
export function foldEvents(events: readonly DownloadEvent[]): DownloadState {
