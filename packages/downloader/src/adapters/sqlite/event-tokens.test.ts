 ...
 * hazard is real here, because `DownloadStarted` is a stored token whose model name is
 * `TryStarted`, while the model name `DownloadRequested` stores as `AcquisitionRequested`.
 */
...
describe('stored-token <-> model-type map', () => {
  ...
  it('refuses to write a type it has no token for, naming it', () => {
    ...
    expect(() => toStoredToken('NotAnEvent' as DownloadEventType)).toThrow(/NotAnEvent/);
  });
});
