 ...
 *
 * Webhook-era settings (WEBHOOK_URLS, WEBHOOK_SECRET, VERDICT_WEBHOOK_SECRET) are deliberately not
 * read: cross-module delivery is the in-process subscription seam (merge-modular-monolith), so an
 * environment still carrying them is simply inert.
 */
