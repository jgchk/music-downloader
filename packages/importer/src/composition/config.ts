 ...
 *
 * Webhook-era settings (INTAKE_WEBHOOK_SECRET, VERDICT_WEBHOOK_URLS, VERDICT_WEBHOOK_SECRET) are
 * deliberately not read: cross-module delivery is the in-process subscription seam
 * (merge-modular-monolith), so an environment still carrying them is simply inert.
 */
...
const envSchema = z.object({
  ...
  /**
   * The DOWNLOADER module's root prefix its delivered `location`s fall under; the intake
   * subscription re-roots them onto INTAKE_ROOT. Absent → cross-module intake is not wired.
   */
...
export interface AppConfig {
  ...
  /** The sender-namespace root for the cross-module intake subscription (absent → not wired). */
...
export function loadConfig(env: NodeJS.ProcessEnv): Result<AppConfig, string> {
  ...
  return ok({
    ...
    intakeSourceRoot: parsed.data.INTAKE_SOURCE_ROOT,
  });
}
