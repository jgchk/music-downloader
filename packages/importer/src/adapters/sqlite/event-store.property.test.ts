...
class AppendCommand implements fc.AsyncCommand<StoreModel, RealStore> {
  ...
  async run(model: StoreModel, real: RealStore): Promise<void> {
    ...
    // The append landed on the stream it was ASKED for. Checked before the model learns anything:
    // otherwise a mis-filed append would teach the model the wrong stream, and `readStream` would
    // then agree with a model that had already absorbed the bug.
    expect(stored.map((row) => row.streamId)).toEqual(stored.map(() => this.streamId));

    for (const row of stored) {
      // Keyed on the *intended* stream, never on what the store reported it as.
