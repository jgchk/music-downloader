...
import { awaitingReviewWithCandidate } from '../../src/domain/import/__fixtures__/import-fixtures.js';
 ...
 * no type checks. Rename the literal on either side alone and the downloader's halt silently
 * degrades into the transient hold path: the seam wedges on the same position forever while
 ...
 * declares) and as CONSUMER (its subscription halts on the kind the *downloader* declares, and on
...
const OWN = seamContractOf('importer');
const PRODUCER = seamContractOf('downloader');
...
describe('the seam-error kinds this module publishes', () => {
  it('renders a permanent payload defect with the kind it declares', () => {
    ...
    const first = awaitingReviewWithCandidate()[0]!;
    const stored: StoredEvent = {
      ...
      type: first.type,
      event: first,
      metadata: { importId: 'imp-1', occurredAt: '2026-07-18T12:00:00.000Z' },
    };
...
describe("this module's subscription over the downloader's feed", () => {
